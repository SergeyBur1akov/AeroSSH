...
class SftpActivity : AppCompatActivity() {
    ...
    private fun downloadSelected() { val e = selectedEntry ?: return; if (e.isDirectory) { Toast.makeText(this, "Select a file", Toast.LENGTH_SHORT).show(); return }
        lifecycleScope.launch { try { binding.transferBar.visibility = View.VISIBLE; binding.transferText.text = "Downloading..."; withContext(Dispatchers.IO) { val safeName = e.name.replace(Regex("[/\\\\:*?\"<>|]"), "_"); val f = File(cacheDir, "dl_${System.currentTimeMillis()}_$safeName"); f.outputStream().use { sftp?.download(e.path, it) }; val uri = FileProvider.getUriForFile(this@SftpActivity, "${packageName}.fileprovider", f); startActivity(Intent.createChooser(Intent(Intent.ACTION_VIEW).apply { setDataAndType(uri, contentResolver.getType(uri)); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION) }, "Open with")); f.delete() }; binding.transferBar.visibility = View.GONE } catch (_: Exception) { binding.transferBar.visibility = View.GONE; Toast.makeText(this@SftpActivity, "Download failed", Toast.LENGTH_SHORT).show() } } }
